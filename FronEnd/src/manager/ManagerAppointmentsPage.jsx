...
/* ── Queue statuses ── */
const STATUS_CFG = {
  scheduled: { label: 'Waiting',    bg: 'bg-blue-100',   text: 'text-blue-700',   icon: CalendarIcon },
  called:    { label: 'Called',     bg: 'bg-purple-100', text: 'text-purple-700', icon: BellRing     },
  ongoing:   { label: 'In Consult', bg: 'bg-yellow-100', text: 'text-yellow-700', icon: PlayCircle   },
  completed: { label: 'Done',       bg: 'bg-green-100',  text: 'text-green-700',  icon: CheckCircle2 },
...
/*
  QUEUE SORT ALGORITHM — Pure FCFS
  1. Appointment-based: sorted by start_time (earlier slot = served first)
  2. Walk-ins: sorted by queue_number (arrival order / registration time)
  3. If both have start_time, earlier wins
  After sorting, reassign queue_number 1…N
*/
const buildSortedQueue = (apts) => {
  const active = apts.filter(a => ['scheduled', 'called', 'ongoing'].includes(a.status));
  const sorted = [...active].sort((a, b) => {
    if (a.start_time && b.start_time) return a.start_time.localeCompare(b.start_time);
    if (a.start_time) return -1;
    if (b.start_time) return 1;
    return (a.queue_number ?? 999) - (b.queue_number ?? 999);
  });
  return sorted.map((apt, idx) => ({ ...apt, queue_number: idx + 1 }));
};

/* Find doctor with shortest active queue */
const getShortestQueueDoctor = (doctors, apts) => {
  ...
  return doctors
    ...
    .sort((a, b) => a.count - b.count)[0]?.doctor ?? null;
};
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            {appointment.queue_number && (
              <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-600 text-white text-xs font-black">
                #{appointment.queue_number}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={k} className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-xs text-gray-700 font-semibold mt-0.5">{v}</p>
              </div>
            ))}
          </div>
          {appointment.reason && (
...
const AppointmentsPage = () => {
  ...
  /*
    Queue: pure FCFS — sorted by appointment time slot, then arrival order.
    No priority levels, no emergency flags.
  */
  const queue = useMemo(() => {
    const todayActive = appointments.filter(a =>
      a.date === TODAY && ['scheduled', 'called', 'ongoing'].includes(a.status)
    );
    return buildSortedQueue(todayActive);
  }, [appointments]);

  /* Per-doctor sub-queues */
  const doctorQueues = useMemo(() => {
    const map = {};
    doctors.forEach(d => {
      map[String(d.id)] = queue.filter(a => String(a.doctor_id) === String(d.id));
    });
    return map;
  }, [doctors, queue]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const rem = list => list.filter(x => x.id !== modal.appointment.id);
      setAppointments(rem);
      setAllApts(rem);
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {[
            ...
            { label: 'In Consult',  value: kpis.ongoing,   Icon: PlayCircle,   bg: 'bg-yellow-50', color: 'text-yellow-600' },
        ...
        {/* TAB BAR — no Book Appointment button */}
        <div className="flex items-center gap-2 flex-wrap">
          {[
            { key: 'table', label: 'Appointments',            Icon: LayoutList },
            { key: 'queue', label: `Queue (${queue.length})`, Icon: Users      },
          ].map(t => (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start justify-between flex-wrap gap-3">
              <div>
                <p className="text-sm font-bold text-gray-700">
                  {queue.length} patient{queue.length !== 1 ? 's' : ''} in queue today
                </p>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {Object.entries(STATUS_CFG)
                .filter(([k]) => ['scheduled','called','ongoing'].includes(k))
                .map(([key, cfg]) => {
                  const Icon  = cfg.icon;
                  const count = queue.filter(a => a.status === key).length;
                  return (
                    <div key={key} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg ${cfg.bg}`}>
                      <Icon className={`w-3.5 h-3.5 ${cfg.text}`} />
                      <span className={`text-xs font-bold ${cfg.text}`}>{cfg.label}</span>
                      <span className="text-xs font-black text-gray-700 ml-1">{count}</span>
                    </div>
                  );
                })}
            </div>

            {/* Multi-doctor workload */}
            {doctors.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2">
                    <Stethoscope className="w-4 h-4 text-blue-600" /> Doctor Queue Load
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    {doctors.map(d => {
                      const dq     = doctorQueues[String(d.id)] || [];
                      const active = dq.find(a => a.status === 'ongoing');
                      return (
                        <div key={d.id} className="border border-gray-100 rounded-xl p-3">
                          ...
                          <p className="text-xs text-gray-400 truncate mb-2">{d.specialization || 'General'}</p>
                          <div className="flex items-end justify-between">
                            <span className="text-2xl font-black text-gray-900">{dq.length}</span>
                            <span className="text-xs text-gray-400 mb-1">in queue</span>
                          </div>
                          {active ? (
                            <div className="mt-1.5 bg-yellow-50 rounded-lg p-1.5">
                              <p className="text-xs text-yellow-700 font-semibold truncate">
                                Now: {patients.find(p => String(p.id) === String(active.patient_id))?.name || '—'}
                              </p>
                            </div>
                          ) : (
                            <div className="mt-1.5 bg-green-50 rounded-lg p-1.5">
            ...
            {queue.length === 0 ? (
              <Card>
                <CardContent className="text-center py-14">
                  ...
                  <p className="text-xs text-gray-300 mt-1">Appointments will appear here once scheduled</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-2">
                {queue.map((apt, idx) => {
                  ...
                  const isCalled  = apt.status === 'called';

                  // Est. wait = patients still waiting ahead × AVG_CONSULT_MINS
                  const aheadWaiting = queue.slice(0, idx).filter(a => a.status === 'scheduled').length;
                  const waitMins     = aheadWaiting * AVG_CONSULT_MINS;

                  return (
                    <Card key={apt.id} className={`border-l-4 ${isOngoing ? 'border-l-yellow-400 ring-1 ring-yellow-200' : isCalled ? 'border-l-purple-400 ring-1 ring-purple-200' : 'border-l-blue-300'}`}>
                      <CardContent className="p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                              ...
                              &nbsp;·&nbsp;{dr?.name || `Doctor #${apt.doctor_id}`}
                              &nbsp;·&nbsp;{apt.appointment_type || 'General'}
                            </p>
                            <p className="text-xs mt-0.5">
                              {isOngoing ? (
                                <span className="text-yellow-600 font-semibold">🟡 In consultation now</span>
                              ) : isCalled ? (
                                <span className="text-purple-600 font-semibold">🔔 Called — please proceed to room</span>
                              ) : idx === 0 ? (
                                <span className="text-green-600 font-semibold">✅ Next up — ready to be called</span>
                              ) : (
                                <span className="text-gray-400">Est. wait: ~{waitMins} min</span>
                              )}
                            </p>
                          </div>
